...
const vcard = 'BEGIN:VCARD\n' 
            ...
            + 'FN:MBUT Admin\n' 
            + 'ORG: Pengembang MBUT;\n' 
            + 'TEL;type=CELL;type=VOICE;waid=6282331033919:+62 823-3103-3919\n' 
            + 'END:VCARD' 
blocked = []   
prefix = '#'
limitawal = 30
memberlimit = 2
cr = '*BOT INI HIDUPNYA DI KENDALIKAN OLEH OWNER👹👹*'
/*************************************/

/******** OWNER NUMBER**********/
const ownerNumber = ["6282331033919@s.whatsapp.net","+6281328141479@s.whatsapp.net"] 
/************************************/
...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `*WOOWW!!!* 😱😱 Si @${num.split('@')[0]}\Masuk grup, Selamat datang di group *${mdata.subject}* yang betah ya di sini`
				...
				teks = `yaaahhhh!!! si @${num.split('@')[0]} Keluar😰😰😰, awas lu join lagi anjing!!!👺👺`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
        ...
        if (levelRole <= 3) {
          ...
          const checkLimit = (sender) => {
           ...
           return false
            ...
            break
                ...
                addKoinUser('6282331033919@s.whatsapp.net', fee)
            ...
            case 'bitly':
               ...
               if (data.error) return reply(data.error)
				...
				case 'bcgc':
					...
					client.sendMessage(from, pok, image, { quoted: mek, caption: `*Nih kak >_<*`})
					await limitAdd(sender)
					break 
					case 'resepmasakan':
					if (!isRegistered) return reply(ind.noregis())
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					const bisa =['Bisa','coba aja','Tidak Bisa','Coba Ulangi']
           ...
           if (isLimit(sender)) return reply(ind.limitend(pusname))
					apakah = body.slice(1)
					const apa =['Iya','Tidak','tidak mungkin','Bisa Jadi','Coba Ulangi']
          ...
          if (!isRegistered) return reply(ind.noregis())
			...
			if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				try {
						if (args.length < 1) return client.sendMessage(from, '𝘂𝘀𝗲?? namanya mana?', text, {quoted: mek})
